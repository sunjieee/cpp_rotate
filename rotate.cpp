...
private:
	...
	const int ROWS = 16;
	const int COLS = 16;
	const int WIDTH_ROWS = 64;
	const int WIDTH_COLS = 64;
	const int CENTRE_ROWS = 31;
	const int CENTRE_COLS = 31;
	const int BIN_NUM = 36;
	const float GAUSSIAN_VALUE = 10.0f;
	const float ORI_RATIO = 0.7f;
...
{
	return 1.0f * ((bin_id - BIN_NUM / 2) * (360 / BIN_NUM) + (180 / BIN_NUM)) / 180 * acos(-1.0);
}
